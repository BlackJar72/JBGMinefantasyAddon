...
public class GenerationHandler implements IWorldGenerator {
	...
	public GenerationHandler() {
		graniteGen1 = new WorldGenMinable(AddonBlocks.granite, 64);
		slateGen1   = new WorldGenMinable(AddonBlocks.slate,   64);
		graniteGen2 = new WorldGenMinable(AddonBlocks.granite, 32);
		slateGen2   = new WorldGenMinable(AddonBlocks.slate,   32);
			...
			for(int j = sz + 15; j >= sz; j--) {
				...
				for(int k = sy; k > 0; k--) {
					if(world.getBlock(i, k, j) == Blocks.stone) {
						world.setBlock(i, k, j, AddonBlocks.granite, 0, 2);
					}
				}
			}				
	}
