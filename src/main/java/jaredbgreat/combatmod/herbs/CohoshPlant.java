...
public class CohoshPlant extends BlockHerb {

	protected CohoshPlant() {
		setBlockName(Info.ID + "-Cohosh");
		setBlockBounds(0.35f, 0f, 0.35f, 0.65f, 0.85f, 0.65f); // ???
        setBlockTextureName(Info.ID + ":Herb/Cohosh");
        setLightOpacity(0);
	}
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, 
    	...
    	return this.beGathered(world, x, y, z, player, Herbs.cohoshItem, Items.shears, 1, false);
    }
    ...
    public void registerBlockIcons(IIconRegister ico) {
    	icons = new IIcon[2];
    	icons[0] = ico.registerIcon(Info.ID + ":Herb/Cohosh");
    	icons[1] = ico.registerIcon(Info.ID + ":Herb/Cohosh_harvested");
    }	
    ...
    public boolean isGoodBiome(BiomeGenBase biome) {
    	return ((biome.rainfall >= 0.5) && (biome.temperature > 0.4f));
    }
